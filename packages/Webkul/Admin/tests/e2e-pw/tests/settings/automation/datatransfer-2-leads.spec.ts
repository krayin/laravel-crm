...
/* -------------------------- Import Management for Leads -------------------------- */
test.describe('Import Management for Leads', () => {
  const csvFileName = 'leads.csv';
  ...
  test('should create a new import and validate records', async ({ adminPage }) => {
    ...
    await adminPage.locator('#import-type').selectOption('leads'); // Adjust selector if multiple checkboxes present
