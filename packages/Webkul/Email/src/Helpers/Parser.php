...
class Parser
{
    ...
    public function extractMultipartMIMEText($part, $source, $encodingType)
    {
        ...
        foreach (array_reverse($boundaries) as $index => $boundary) {
            ...
            foreach ($processedEmailSegments as $emailSegment) {
                ...
                if (stripos($emailSegment, 'content-type: text/plain;') !== false || stripos($emailSegment, 'content-type: text/html;') !== false) {
    ...
    public function getMessageBody($type = 'text')
    {
        ...
        if (in_array($type, array_keys($mime_types))) {
            foreach ($this->parts as $key => $part) {
                if (in_array($this->getPart('content-type', $part), $mime_types) && $this->getPart('content-disposition', $part) != 'attachment') {
                    ...
                    if ($this->getPart('content-type', $part) == 'text/plain') {
                        ...
                        if (!empty($mail_part_addresses[0]['address']) && false !== strrpos($mail_part_addresses[0]['address'], 'pcsms.com')) {
                    ...
                    } elseif ($this->getPart('content-type', $part) == 'multipart/mixed' || $this->getPart('content-type', $part) == 'multipart/related') {
    ...
    public function getAttachments()
    {
        ...
        $non_attachment_types = array('text/plain', 'text/html', 'text/plain; (error)');
        $nonameIter = 0;
        ...
        foreach ($this->parts as $part) {
            ...
            if (in_array($disposition, $dispositions) === true && isset($filename) === true) {
                ...
                if(!$contentidAttachments && $disposition == "inline" && !strpos($this->getPart('content-type', $part), 'image/') && !stripos($filename,"noname") == false) {
                    //skip
                } else {
    ...
    public function saveAttachments($attach_dir)
    {
        ...
        foreach ($attachments as $attachment) {
            ...
            $fileService->setRequiredFileSystem($fileSystem);
            $fileService->entity = $this->entity;
    ...
    public function extractMultipartMIMEAttachments()
    {
        ...
        foreach ($attachmentCollection as $attachmentDetails) {
            ...
            $processedAttachmentCollection[] = new Attachment($resourceDetails['fileName'], $resourceDetails['contentType'], $temp_fp, $resourceDetails['contentDisposition'], $resourceDetails['contentId'], []);
        }
